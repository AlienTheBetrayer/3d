...
- `@repo/api-schemas` (`packages/api-schemas`) - Zod schemas derived from `@repo/db` tables via `drizzle-zod`
...
3. `@repo/api-schemas` updates itself - `drizzle-zod` runs at import time, there is no codegen step
...
Zod row schemas are imported from `@repo/api-schemas`
